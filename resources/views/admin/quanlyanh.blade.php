...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <button id="openAddModal" class="bg-teal-500 text-white px-4 py-2 rounded-lg hover:bg-teal-600 transition flex items-center">
                ...
                @forelse ($images as $item)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-800">
                            <img src="{{ asset('assets/images/' . $item->image_url) }}" alt="Image" class="h-10 w-10 rounded object-cover"
                        ...
                        <td class="px-6 py-4 text-sm">
                            <button class="text-teal-500 hover:text-teal-600 mr-4 edit-image" data-id="{{ $item->id }}">
    ...
    <div class="bg-white rounded-xl p-6 w-full max-w-md">
        ...
        <form id="imageForm" enctype="multipart/form-data" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="file" name="image" accept="image/*" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
...
<div id="notificationModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-xl p-6 w-full max-w-md">
        ...
        <div class="flex justify-end">
            <button id="closeNotificationModal" class="px-4 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition">Đóng</button>
        </div>
    </div>
</div>
...
<div id="confirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-xl p-6 w-full max-w-md">
        ...
        <div class="flex justify-end space-x-2">
            <button id="cancelDelete" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition">Hủy</button>
            <button id="confirmDelete" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">Xóa</button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    imageModal.addEventListener('click', (e) => e.target === imageModal && imageModal.classList.add('hidden'));

    // Close Notification
    closeNotificationModal.addEventListener('click', closeNotificationAndReload);
    notificationModal.addEventListener('click', (e) => e.target === notificationModal && closeNotificationAndReload());
    ...
    imageForm.addEventListener('submit', async (e) => {
        ...
        const url = currentImageId
            ? `{{ route('admin.images.update', ['id' => ':id']) }}`.replace(':id', currentImageId)
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`{{ route('admin.images.show', ['id' => ':id']) }}`.replace(':id', currentImageId), {
                    method: 'GET',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json'
                ...
                if (response.ok && image.success) {
                    if (!image.data || !image.data.product_id) {
                        showAlert('Dữ liệu ảnh không đầy đủ, vui lòng kiểm tra database!', 'error');
                        return;
                    }

                    const form = imageForm;
                    form.querySelector('[name="product_id"]').value = image.data.product_id || '';
                    form.querySelector('[name="image_url"]').value = image.data.image_url || '';
    ...
    confirmDeleteModal.addEventListener('click', (e) => e.target === confirmDeleteModal && confirmDeleteModal.classList.add('hidden'));

    // Confirm Delete
    confirmDelete.addEventListener('click', async () => {
        try {
            const response = await fetch(`{{ route('admin.images.destroy', ['id' => ':id']) }}`.replace(':id', currentImageId), {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
