    ...
    <main class="container product_section_container mt-5 pt-5">
        <div class="row product_section">
            ...
            <nav aria-label="breadcrumb" class="breadcrumbs mb-4">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('products.home') }}">Home</a></li>
            ...
            <aside class="col-lg-3 sidebar">
                <div class="sidebar_section mb-4 filter-card">
                        ...
                        @foreach ($categories as $category)
                            <div class="form-check">
                                <input class="form-check-input custom-checkbox" type="checkbox"
                                    value="{{ $category->category_id }}" id="category-{{ $category->category_id }}"
                    ...
                    <button id="clear-category-filter" class="btn btn-outline-secondary btn-sm mt-2">Xóa bộ lọc danh
                        mục</button>
                </div>
                ...
                <div class="sidebar_section mb-4 filter-card">
                    ...
                    <div class="price-range-filter mt-3">
                        ...
                        <div class="d-flex align-items-center mb-2">
                            <input type="number" id="price-min" class="form-control me-2" placeholder="Tối thiểu"
                                min="0" step="100000">
                            <span>-</span>
                            <input type="number" id="price-max" class="form-control ms-2" placeholder="Tối đa"
                                min="0" step="100000">
                        </div>
                ...
                <div class="sidebar_section filter-card">
                    ...
                    <ul class="checkboxes list-unstyled brand-list">
                        @foreach ($brands as $brand)
                            <div class="form-check">
                                <input class="form-check-input custom-checkbox" type="checkbox" value="{{ $brand->slug }}"
                                    id="brand-{{ $brand->id }}" {{ $brandSlug == $brand->slug ? 'checked' : '' }}
                                    onchange="window.location.href='{{ route('products.index', [$brand->slug, $categorySlug]) }}'">
                    ...
                    <a href="{{ route('products.index', [null, $categorySlug]) }}"
                        class="btn btn-outline-secondary btn-sm mt-2">Xóa bộ lọc thương hiệu</a>
                </div>
            </aside>
            ...
            <section class="col-lg-9 main_content">
                <div class="products_iso">
                    <!-- Product Sorting -->
                    <div
                        class="product_sorting_container product_sorting_container_top d-flex justify-content-between align-items-center mb-4">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu sorting_type">
                                <li class="type_sorting_btn" data-isotope-option='{ "sortBy": "original-order" }'><a
                                        class="dropdown-item" href="#">Tất Cả</a></li>
                                <li class="type_sorting_btn" data-isotope-option='{ "sortBy": "price" }'><a
                                        class="dropdown-item" href="#">Giá</a></li>
                                <li class="type_sorting_btn" data-isotope-option='{ "sortBy": "name" }'><a
                                        class="dropdown-item" href="#">Tên Sản phẩm</a></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div class="product-grid row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                        @foreach ($products as $item2)
                            <div
                                class="col product-item category-{{ $item2->category_id }} brand-{{ $item2->brand_id }}">
                                <div class="card product discount product_filter h-100">
                                    <div class="product_image">
                                        <img src="{{ asset('assets/images/' . $item2->image_url) }}" class="card-img-top"
                                    ...
                                    <div class="card-body product_info text-center">
                                        <h6 class="card-title product_name"><a
                                                href="{{ route('products.show', $item2->product_id) }}">{{ $item2->product_name }}</a>
                                        </h6>
                                        <div class="product_price">{{ number_format($item2->price, 0, ',', '.') }}đ
                                            <span></span>
                                        </div>
                                    </div>
                                    <div class="card-footer add_to_cart_button">
                                        <a href="{{ route('cart.add', $item2->product_id) }}"
                                            class="btn red_button w-100">Add to Cart</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div
                        class="product_sorting_container product_sorting_container_bottom d-flex justify-content-between align-items-center mt-4">
