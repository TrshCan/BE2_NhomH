            ...
            <div class="col">
                <div class="breadcrumbs">
                    <ul>
                        ...
                        <li><a href="{{ url('/categories/headsets') }}"><i class="fa fa-angle-right" aria-hidden="true"></i>
                                Headsets</a></li>
                            ...
                            <div class="single_product_thumbnails">
                                <ul class="list-unstyled">
                                    @foreach ($product->images as $index => $image)
                                        <li>
                                            <img src="{{ asset('assets/images/' . $image->image_url) }}" alt=""
                                                class="{{ $index === 0 ? 'active' : '' }}"
                                                data-image="{{ asset('assets/images/' . $image->image_url) }}">
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <div class="container my-3">
                            <div class="tabs_container d-flex justify-content-center">
                                <ul class="tabs nav">
                                    ...
                                    <li class="tab mx-2" data-active-tab="tab_3"><span>Reviews ({{ $count }})</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-lg-5 desc_col">
                                    ...
                                    @if ($product->images->isNotEmpty())
                                        <div class="tab_image">
                                            <img src="{{ asset('assets/images/' . $product->images[0]->image_url) }}"
                                ...
                                <div class="col-lg-5 offset-lg-2 desc_col">
                                    @if ($product->images->count() > 1)
                                        <div class="tab_image">
                                            <img src="{{ asset('assets/images/' . $product->images[1]->image_url) }}"
                                                alt="{{ $product->product_name }}">
                                        </div>
                                    @endif
                                    ...
                                    @if ($product->images->count() > 2)
                                        <div class="tab_image desc_last">
                                            <img src="{{ asset('assets/images/' . $product->images[2]->image_url) }}"
                                                alt="{{ $product->product_name }}">
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="col-lg-6 reviews_col">
                                        ...
                                        @foreach ($reviews as $review)
                                            <div class="user_review_container d-flex flex-column flex-sm-row gap-3">
                                                ...
                                                <div class="review" style="width: 55%;">
                                                    <div class="review_date">{{ $review->created_at->format('d M Y') }}
                                                    </div>

                                                    <p>
                                                        {{ strlen($review->comment) > 55 ? substr($review->comment, 0, 35) . '...' : $review->comment }}
                                                    </p>
                                                    <img src="{{ file_exists(public_path($review->image)) ? asset($review->image) : asset('assets/images/default.jpg') }}" alt="review Image"
                                    ...
                                    <div class="col-lg-6 add_review_col my-4 mx-auto">
                                        <div class="add_review">
                                            <form id="review_form"
                                                action="{{ route('reviews.form', ['product_id' => $product->product_id]) }}"
                                                method="GET">
                                                @csrf

                                                <div class="text-left text-sm-right">
                                                    <button id="review_submit" type="submit"
                                                        class="red_button review_submit_btn">Đánh giá</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
